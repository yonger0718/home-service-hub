...
// Keep a fresh response per request/viewport. Confirmation transfers the stored
// net amount; it must not change the estimate or its cost-based percentage.
function summaryForReceipt(confirmed) {
  const result = structuredClone(summary);
  if (!confirmed) return result;
  const amount = Number(row.amount);
  result.total_pending_dividends_net -= amount;
  result.total_dividends += amount;
  result.total_cash_twd += amount;
  result.total_assets_twd += amount;
  result.market_totals.TW.pending_dividends_net -= amount;
  result.market_totals.TW.recorded_dividends += amount;
  const tw = result.holdings.find(h => h.symbol === row.symbol && h.market === row.market);
  tw.pending_dividends_net -= amount;
  tw.total_dividends += amount;
  tw.total_pnl_with_dividend += amount;
  return result;
}
  ...
  try {
    ...
    for (const [label,width,height] of [['desktop',1440,1100],['mobile',390,844]]) {
      ...
      await page.route('**/api/**', async route => {
        const url = new URL(route.request().url());
        let body = [];
        if (url.pathname.endsWith('/summary')) body = summaryForReceipt(confirmed);
      ...
      const estimateBefore = await page.locator('.estimated-total').innerText();
      const ratioBefore = await page.locator('app-bento').filter({has:page.locator('.estimated-total')}).locator('app-pct-badge').innerText();
      const holdingBefore = await page.locator('.stock-stats').first().innerText();
      assert.match(await page.locator('.estimated-breakdown').innerText(), /已記錄股利\s*\$100/);
      assert.match(await page.locator('.estimated-breakdown').innerText(), /待收股利\s*\$3,090/);
      ...
      await page.goto('http://synthetic.test/hub/portfolio');
      await page.locator('.estimated-total').waitFor();
      assert.equal(await page.locator('.estimated-total').innerText(), estimateBefore);
      assert.equal(await page.locator('app-bento').filter({has:page.locator('.estimated-total')}).locator('app-pct-badge').innerText(), ratioBefore);
      assert.equal(await page.locator('.stock-stats').first().innerText(), holdingBefore);
      assert.match(await page.locator('.estimated-breakdown').innerText(), /已記錄股利\s*\$3,190/);
      assert.match(await page.locator('.estimated-breakdown').innerText(), /待收股利\s*\$0(?:\s|$)/);
      await page.locator('.stock-row').first().click();
      await page.locator('.detail-panel').waitFor();
      const detailsAfter = await page.locator('.detail-panel').innerText();
      assert.match(detailsAfter, /已記錄股利\s*\$3,190/);
      assert.match(detailsAfter, /待收股利\s*\$0(?:\s|$)/);
      assert.match(detailsAfter, /預估含息損益\s*\$12,707/);
      await page.screenshot({path:path.join(output,`${label}-confirmed-dashboard.png`),fullPage:true});
      await page.locator('.market-tabs').getByRole('radio',{name:'TW',exact:true}).click();
      await page.locator('.estimated-total').filter({hasText:/12,707/}).waitFor();
      assert.match(await page.locator('.estimated-breakdown').innerText(), /已記錄股利\s*\$3,190/);
      assert.match(await page.locator('.estimated-breakdown').innerText(), /待收股利\s*\$0(?:\s|$)/);
      assert.match(await page.locator('app-bento').filter({has:page.locator('.estimated-total')}).locator('app-pct-badge').innerText(), /12.71%/);
      await page.screenshot({path:path.join(output,`${label}-confirmed-tw.png`),fullPage:true});
      evidence.checks.push(`${label}: main/holding estimate, TW/US/All filter, detail expansion, no overflow, pending-only confirmation and dashboard revisit: pending 3090→0, recorded 100→3190, unchanged All/TW/holding estimate and ratio`);
