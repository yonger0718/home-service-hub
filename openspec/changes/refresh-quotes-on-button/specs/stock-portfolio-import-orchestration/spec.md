...
- **WHEN** `POST /api/portfolio/refresh-quotes` is called and no symbol has positive net holdings
...
- **WHEN** `POST /api/portfolio/refresh-quotes` is called while `_RECALC_LOCK` is already held by another chain
...
- **WHEN** `POST /api/portfolio/refresh-quotes` is called twice in succession on the same calendar day with the same holdings
