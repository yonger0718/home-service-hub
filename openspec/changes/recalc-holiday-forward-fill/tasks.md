...
- [ ] 7.2 Manual smoke: trigger `POST /api/portfolio/imports/recalc` over a range covering known stale dates (e.g. `{"start_date":"2022-01-01","end_date":"2023-12-31"}`). Verify `MV=0 cost>0` rows disappear (`docker exec -e PGPASSWORD=stonk stonk-postgres-1 psql -U stonk -d stock_portfolio_db -c "SELECT COUNT(*) FROM portfolio_snapshot WHERE total_market_value=0 AND total_cost>0"` → 0). Verify forward-fill rows exist on the 春節 cluster dates.
- [ ] 7.3 Manual smoke: load `/hub/portfolio` chart, confirm 總市值 series no longer drops to zero on any historical date.
